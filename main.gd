...
func _unhandled_input(event):
	if event is InputEventMouseButton:
		...
		if !event.pressed:
			grabbing = false
			#compicactus_anim.play(Array(compicactus_anim.get_animation_list()).pick_random())
...
#	var rayVector = camera.project_ray_normal(cursor.position)
#	var rayPoint = camera.project_ray_origin(cursor.position)
#	Global.cursor3d_pos = Common.planeRayIntersection(rayVector,rayPoint, Vector3.ZERO, Vector3.UP)
